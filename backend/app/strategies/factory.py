...
STRATEGY_META: Dict[str, Dict[str, Any]] = {
    ...
    StrategyName.TABLE_CONFIG: {
        "label": "可配置表格分块",
        "description": "分析表格结构后，自定义数据列（可命名）+ 选择元数据列，适用于任意结构的复杂表格",
        "default_params": {
            "data_columns": [],
            "metadata_cols": [],
            "skip_rows": 1,
        ...
        "param_schema": [
            ...
            },
            {
                ...
                "label": "按员工聚合",
                "type": "switch",
                "description": "开启后按 data_columns 中分组关键词列的值分组，将同一组的所有数据列条目汇总为一条 Chunk",
            },
            {
                "key": "group_column_keyword",
                "label": "分组关键词",
                "type": "text",
                "placeholder": "姓名",
                "description": "员工聚合模式下，用于标识分组列的关键词。列名包含该关键词的 data_column 被视为分组依据列。可自定义，如「员工」、「负责人」、「name」等。",
            },
        ],
    },
}
