...
│           └── table_chunker.py          # HTML 表格 + 复杂多维 + 可配置表格分块引擎
└── frontend/
    ├── package.json
    ├── vite.config.ts
    ├── tsconfig.json
...
| **可配置表格分块** | **`ConfigTableChunker`** | **自定义列映射的灵活表格分块，用户手动指定数据列和元数据列，支持按员工聚合模式** |

### 可配置表格分块 — 使用指南

`table_config` 策略适用于**任意结构**的表格，不依赖自动识别，完全由用户手动配置列映射。

#### 适用场景

- 宽表结构：每行是一个考核指标，每名员工占据一组数据列
- 自定义考核表：列顺序和命名不固定，用户需要精确控制哪些列作为数据、哪些作为元数据
- 多表格文档：文档包含多个不同结构的表格，需要为每个表格单独配置

#### 快速上手

**第一步：上传文档并分析表格结构**

点击「分析表格结构」按钮，后端将解析文档中所有 HTML `<table>` 表格，显示每个表格的行列数和各列前 3 行样本数据，并自动建议列角色（员工/评分/理由/维度/未知）。

**第二步：选择表格**

从「选择表格」下拉框中选择要处理的表格（文档可能包含多个表格，每个表格需单独配置）。

**第三步：配置数据列**

数据列是 Chunk 内容的主要构成部分。每行配置包含：

| 字段 | 说明 |
|------|------|
| **列名** | 数据列名称，会出现在 Chunk 内容中（如"评分"、"评分理由"） |
| **列号** | 从列预览中选择对应的 Excel 列字母（A、B、C...），决定从哪一列读取数据 |
| **行号**（可选） | 从 Excel 第几行读取数据，留空则从当前数据行读取。**填 1 表示 Excel 第 1 行** |
| **分组值**（员工聚合模式下） | 当列名包含分组关键词时显示，手动输入分组名称（如员工姓名） |

**第四步：配置元数据列（可选）**

元数据列会作为每条评分条目的上下文信息，附加到 Chunk 的 metadata 中。选中后，每条评分条目会附带其所在行的元数据值。

**第五步：保存配置**

点击「保存当前表格配置」按钮，切换表格后配置不会丢失。如需修改其他表格，保存后再切换，切回时自动恢复。

#### 按员工聚合模式（employee_mode）

针对「宽表」场景（一行是一个考核项，多列对应不同员工的数据），开启「按员工聚合」开关后：

1. **分组关键词**：默认关键词为`姓名`，可自定义为任意字符串（如`员工`、`负责人`、`Employee`）
2. **分组方式**：列名包含分组关键词的数据列被视为分组依据列，两个分组列之间的数据列属于前一个员工组
3. **输出规则**：同一员工的所有评分条目合并为一条 Chunk，按行组织为自然语言记录
4. **元数据**：每条评分条目附带其所在行的元数据（如考核模块、考核指标、权重、评分理由等）

**示例配置（员工聚合模式）：**

假设表格结构如下（一行一个指标，每名员工占两列）：

| 考核模块 | 考核指标 | 姓名：张涛 | 评分 | 评分理由 | 姓名：李强 | 评分 | 评分理由 |
|---------|---------|-----------|------|---------|-----------|------|---------|

配置方式：
- 添加数据列 `姓名`（分组关键词列），手动输入员工姓名
- 添加数据列 `评分`，指定行号和列号
- 添加数据列 `评分理由`，指定行号和列号
- 选择元数据列（A: 考核模块、B: 考核指标）
- 开启「按员工聚合」开关
...
```
姓名：邹想中。
考核记录1：考核模块为「工作达成 情况（70%）」，
          考核指标为「团队月度工作完成情况」，
          考核参考数据来源为「版本开发为主：...」，
          权重为「0.08」，
          评分理由为「1、亚信V2.0...」，
          评分为「5」。
考核记录2：考核模块为「工作达成 情况（70%）」，
          考核指标为「团队月度工作完成情况」，
          考核参考数据来源为「项目支撑为主：...」，
          权重为「0.07」，
          评分理由为「任子行...」，
...
每个表格的数据列和元数据列配置可独立保存，切换表格后自动恢复：
- **保存当前表格配置**：将当前配置保存到内存缓存
- **切换表格**：自动恢复已保存的配置，未保存的表格清空
- **更新配置**：修改后再次点击保存更新已有配置

> 注意：配置仅在当前会话中有效，刷新页面后需要重新配置。
| **可配置表格分块** | **`ConfigTableChunker`** | **自定义列映射的灵活表格分块，用户手动指定数据列和元数据列，支持按员工聚合模式** |
...
BaseChunker (抽象接口)
    ...
    ├── HTMLTableChunker
    ├── ComplexHTMLTableChunker
    └── ConfigTableChunker              # 可配置表格分块
